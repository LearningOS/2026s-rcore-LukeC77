...
lazy_static! {
    ...
    pub static ref TASK_MANAGER: TaskManager = {
        ...
        let mut tasks = [TaskControlBlock {
            ...
            syscall_count: [0; 500], // yifan 2026/5/3: initialize syscall count to zero
        }; MAX_APP_NUM];
...
impl TaskManager {
    ...
    /// yifan 2026/5/3: Increment syscall count for the current task
    ...
    /// yifan 2026/5/3: Get syscall count for the current task
    fn get_syscall_count(&self, syscall_id: usize) -> isize {
        let inner = self.inner.exclusive_access();
        let current = inner.current_task;
        inner.tasks[current].syscall_count[syscall_id] as isize
    }
}
